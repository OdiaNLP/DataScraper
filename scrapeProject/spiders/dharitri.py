...
class DharitriSpider(CrawlSpider):
    name = "dharitri"
    start_urls = [
        f"https://www.dharitri.com/{year}/{month:02}/"
    ...
    rules = (
        ...
        Rule(
            LinkExtractor(restrict_css="a.text-black"),
            follow=True,
            callback="parse_item",
        ),
    )
    ...
    def parse_item(self, response):
        ...
        l.add_xpath(
            "header",
            "//header[@class='cono']//h1/text()",
            MapCompose(lambda text: text.strip()),
            Join(),
        )
        l.add_xpath(
            "content",
            "//div[@class='p-t-20']/p//text()",
            MapCompose(lambda text: text.strip()),
            Join(),
        )
        return l.load_item()
